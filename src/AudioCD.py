...
class AudioCD:
    def __init__(self, Fs, configuration, max_interpolation):
        ...
        self.scaled_quantized_padded_original = (
            []
        )  # Reference to compare the output of readCD to
    ...
    def save_and_play_music(self, left_channel, right_channel, wav_file, bool_play=1):
        ...
        assert (
            len(np.shape(left_channel)) == 1
            and len(np.shape(right_channel)) == 1
            and type(left_channel) is np.ndarray
            and type(right_channel) is np.ndarray
        ), "the left and right channel must be 1D numpy arrays"
    ...
    def writeCd(self, audiofile):
        ...
        assert (
            np.shape(audiofile)[1] == 2 and type(audiofile) is np.ndarray
        ), "audiofile must be a 2D numpy array with 2 columns"
        ...
        xlr16 = np.reshape(
            np.transpose(x), (-1, 1), order="F"
        )  # serialize left and right audio channel

        xlr8 = self.typecast_8(xlr16)  # split into 8 bit words

        xlr8_padded = np.hstack(
            (xlr8, np.zeros((24 - (np.fmod(xlr8.size - 1, 24) + 1))))
        )  # pad with zeros to fill an integer number of frames
        ...
        self.scaled_quantized_padded_original = self.udecode(
            y
        )  # Reference to compare the output of readCD to
    ...
    def scratchCd(self, length_scratch, location_scratch):
        ...
        self.cd_bits[
            location_scratch
            - 1 : min(location_scratch - 1 + length_scratch, (self.cd_bits).size)
        ] = 0
    ...
    def readCd(self):
        ...
        if self.configuration == 0:  # no CIRC
            ...
            erasure_flags = np.transpose(
                np.logical_or(erasure_flags[0, :], erasure_flags[1, :])
            )
        ...
        assert (
            np.shape(audio_out)[1] == 2 and type(audio_out) is np.ndarray
        ), "audio_out must be a 2D numpy array with 2 columns"
        assert (
            np.shape(interpolation_flags)[1] == 2
            and type(interpolation_flags) is np.ndarray
    ...
    def interpolator(self, input, erasure_flags_in):
        ...
        erasure_burst = np.zeros(
            erasure_flags_in.size, dtype="B"
        )  # Number of consecutive erasures
        ii = np.where(np.diff(erasure_flags_in) == 1)[0] + 1

        if len(ii) != 0:
            erasure_burst[ii] = (
                np.where(np.diff(erasure_flags_in) == -1)[0] - np.asarray(ii) + 1
            )
            temp = np.where(
                (erasure_burst > 0) & (erasure_burst <= self.max_interpolation)
            )[0]
            if len(temp) > 0:
                for i in temp:
                    output[i : i + erasure_burst[i]] = np.maximum(
                        np.zeros(erasure_burst[i]),
                        np.minimum(
                            (2**16 - 1) * np.ones(erasure_burst[i]),
                            (
                                np.round(
                                    float(output[i - 1])
                                    + np.arange(0, erasure_burst[i])
                                    * (
                                        float(output[i + erasure_burst[i]])
                                        - float(output[i - 1])
                                    )
                                    / (erasure_burst[i] + 1)
                                )
        ...
        assert (
            len(np.shape(interpolation_failed)) == 1
            and type(interpolation_failed) is np.ndarray
        ), "interpolation_failed must be a 1D numpy array"
